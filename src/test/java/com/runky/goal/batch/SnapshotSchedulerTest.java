...
class SnapshotSchedulerTest {
    ...
    @Test
    @DisplayName("스냅샷 생성 배치가 중복 실행되더라도, 스냅샷은 1개만 생성된다.")
    void createSnapshots_Duplicate() throws Exception {
        MemberGoal memberGoal1 = MemberGoal.from(1L);
        memberGoal1.updateGoal(new BigDecimal("10.01"));
        goalRepository.save(memberGoal1);
        MemberGoal memberGoal2 = MemberGoal.from(2L);
        ...
        weeklyGoalSnapshotJobTest.launchJob(jobParameters1);
        weeklyGoalSnapshotJobTest.launchJob(jobParameters2);
    ...
    void checkCrewGoalAchieve() throws Exception {
        ...
        cloverRepository.save(Clover.of(4L));
        ...
        Clover clover4 = cloverRepository.findByUserId(4L).orElseThrow();
        assertAll(
                () -> assertThat(clover1.getCount()).isEqualTo(3L),
                () -> assertThat(clover2.getCount()).isEqualTo(3L),
                () -> assertThat(clover3.getCount()).isEqualTo(3L),
                () -> assertThat(clover4.getCount()).isEqualTo(0L)
        );
    }

    @Test
    @DisplayName("개인 목표 달성 배치가 중복으로 수행되더라도, 보상은 1회만 지급된다.")
    void checkGoalAchieve_Duplicate() throws Exception {
        LocalDate snapshotDate = LocalDate.of(2025, 8, 25);
        cloverRepository.save(Clover.of(1L));
        MemberGoalSnapshot memberGoalSnapshot1 = new MemberGoalSnapshot(1L, new Goal(BigDecimal.TEN), false,
                snapshotDate.minusWeeks(1));
        memberGoalSnapshot1.addDistance(new BigDecimal("10.00"));
        memberGoalSnapshot1.achieve();
        goalRepository.saveAll(List.of(memberGoalSnapshot1));
        ...
        weeklyGoalAchieveJobTest.launchJob(jobParameters1);
        weeklyGoalAchieveJobTest.launchJob(jobParameters2);
    ...
    @Test
    @DisplayName("크루 목표 달성 배치가 중복으로 수행되더라도, 보상은 1회만 지급된다.")
    void checkCrewGoalAchieve_Duplicate() throws Exception {
        Crew crew = Crew.of(new CrewCommand.Create(1L, "crew1"), new Code("code12"));
        crew.joinMember(2L);
        crew.joinMember(3L);
        crew.joinMember(4L);
        ...
        weeklyCrewGoalAchieveJobTest.launchJob(jobParameters1);
        weeklyCrewGoalAchieveJobTest.launchJob(jobParameters2);
