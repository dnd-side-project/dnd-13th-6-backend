...
public class GoalService {
	...
	public void updateDistances(GoalCommand.UpdateDistance cmd) {
		MemberGoalSnapshot memberSnapshot = goalRepository.findSnapshotWithLock(cmd.memberId(),
				WeekUnit.from(cmd.date()))
			.orElseThrow(() -> new GlobalException(GlobalErrorCode.NOT_FOUND));
		memberSnapshot.addDistance(cmd.distance());
		List<CrewGoalSnapshot> crewSnapshots = goalRepository.findAllCrewSnapshotsWithLock(cmd.crewIds(),
			WeekUnit.from(cmd.date()));
	...
	@Transactional
	public void cleanUp(GoalCommand.Clean command) {
		goalRepository.deleteAllMemberSnapshot(command.memberId());
		goalRepository.deleteMemberGoal(command.memberId());
	}
}
